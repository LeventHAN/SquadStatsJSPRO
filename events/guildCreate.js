	...
	async run (guild) {
		...
		if(this.client.config.proMode){
			if((!this.client.config.proUsers.includes(guild.ownerID) || this.guilds.filter((g) => g.ownerID === guild.ownerID) > 1) && guild.ownerID !== this.client.config.owner.id){
				this.client.logger.log(guild.ownerID+" tried to invite SquadStatJS v3 on its server.");
				return guild.leave();
			}
		}
		...
		const text = "Bot did join to **"+guild.name+"**, with **"+guild.members.cache.filter((m) => !m.user.bot).size+"** users (and "+guild.members.cache.filter((m) => m.user.bot).size+" bots)";
