		...
		if (guildData.plugins.goodbye.enabled) {
			...
			if (channel) {
				...
				if (guildData.plugins.goodbye.withImage) {
					...
					ctx.font = applyText(
						canvas,
						member.guild.translate("administration/goodbye:IMG_GOODBYE", {
					...
					ctx.font = "22px Bold";
					ctx.fillText(
						member.guild.translate("administration/goodbye:IMG_NB", {
					...
					ctx.lineWidth = 15;
					ctx.strokeText(
						member.guild.translate("administration/goodbye:TITLE"),
						canvas.width - 620,
						canvas.height - 330
					);
