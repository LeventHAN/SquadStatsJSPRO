		...
		if (status === "create") {
			...
			backup
				...
				.then((backup) => {
					...
					message.author
						.send(
							message.translate("administration/backup:SUCCESS_PRIVATE", {
								backupID: backup.id,
							})
						)
						.catch(() => {
			...
			backup
				...
				.then(async () => {
					...
					message.author.send(
						message.translate("administration/backup:START_LOADING")
					);
					// Load the backup
					backup
						...
						.then(() => {
							...
							message.author.send(
								message.translate("administration/backup:LOAD_SUCCESS")
							);
						})
						.catch((err) => {
							Sentry.captureException(err);
			...
			backup
				...
				.then(async (backupInfos) => {
					const embed = new Discord.MessageEmbed()
						...
						.addField(
							message.translate("administration/backup:TITLE_ID"),
							backupInfos.id,
							true
						)
						// Displays the server from which this backup comes
						.addField(
							message.translate("administration/backup:TITLE_SERVER_ID"),
							backupInfos.data.guildID,
							true
						)
						// Display the size (in mb) of the backup
						.addField(
							message.translate("administration/backup:TITLE_SIZE"),
							backupInfos.size + " mb",
							true
						)
						// Display when the backup was created
						.addField(
							message.translate("administration/backup:TITLE_CREATED_AT"),
							message.printDate(new Date(backupInfos.data.createdTimestamp)),
							true
						)
