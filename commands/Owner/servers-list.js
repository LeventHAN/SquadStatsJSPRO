...
class ServersList extends Command {
	...
	async run(message, args, data) {
		...
		let description =
			`${message.translate("common:SERVERS")}: ${
		...
		const embed = new Discord.MessageEmbed()
			...
			.setTitle(
				`${message.translate("common:PAGE")}: ${page}/${Math.ceil(
		...
		collector.on("collect", async (reaction) => {
			if (reaction._emoji.name === "⬅") {
					...
					this.client.guilds.cache
						...
						.map(
							(r, i) =>
								`**${i + 1}** - ${r.name} | ${
									r.memberCount
								} ${message.translate("common:MEMBERS")}`
						)
						.slice(i0, i1)
						.join("\n");
				...
				embed
					.setTitle(
						`${message.translate("common:PAGE")}: ${page}/${Math.round(
							this.client.guilds.cache.size / 10
						)}`
					)
					.setDescription(description);
