	...
	async run(message, args, data) {
		...
		if (banCount) {
			if (sanctions >= banCount) {
				member.send(
					message.translate("moderation/ban:BANNED_DM", {
				...
				embed
					.setAuthor(
						message.translate("moderation/ban:CASE", {
		...
		if (kickCount) {
			if (sanctions >= kickCount) {
				member.send(
					message.translate("moderation/kick:KICKED_DM", {
						username: member.user,
						moderator: message.author.tag,
						server: message.guild.name,
						reason,
				...
				embed
					.setAuthor(
						message.translate("moderation/kick:CASE", {
							count: data.guild.casesCount,
						})
					)
					.setColor("#e88709");
		...
		member.send(
			message.translate("moderation/warn:WARNED_DM", {
				username: member.user.tag,
				server: message.guild.name,
				moderator: message.author.tag,
				reason,
		...
		embed
			.setAuthor(
				message.translate("moderation/warn:CASE", {
