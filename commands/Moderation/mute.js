...
class Mute extends Command {
	...
	async run(message, args, data) {
		...
		member.send(
			message.translate("moderation/mute:MUTED_DM", {
		...
		if (data.guild.plugins.modlogs) {
			...
			const embed = new Discord.MessageEmbed()
				.setAuthor(
					message.translate("moderation/mute:CASE", {
				...
				.addField(
					message.translate("common:EXPIRY"),
					message.printDate(new Date(Date.now() + ms(time))),
					true
				)
				.setColor("#f44271");
			channel.send(embed);
