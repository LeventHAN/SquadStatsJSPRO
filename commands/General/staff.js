	...
	async run(message, args, data) {
		...
		const moderators = message.guild.members.cache.filter(
			(m) =>
				...
				m.permissions.has("MANAGE_MESSAGES") &&
				!m.user.bot
		);
		const embed = new Discord.MessageEmbed()
			.setAuthor(
				message.translate("general/staff:TITLE", {
			...
			.addField(
				...
				administrators.size > 0
					? administrators
						.map(
							(a) =>
								`${this.client.customEmojis.status[a.presence.status]} | ${
									a.user.tag
								}`
						)
						.join("\n")
			...
			.addField(
				...
				moderators.size > 0
					? moderators
						.map(
							(m) =>
								`${this.client.customEmojis.status[m.presence.status]} | ${
									m.user.tag
								}`
						)
						.join("\n")
