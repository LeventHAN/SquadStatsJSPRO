...
class Lyrics extends Command {
	...
	async run(message, args, data) {
		...
		const embed = new Discord.MessageEmbed()
			.setAuthor(
				message.translate("music/lyrics:LYRICS_OF", {
					songName,
				})
			)
		...
		try {
			...
			const songLink = `https://musixmatch.com${$(
			...
			if (lyrics.length > 2048) {
				lyrics =
					...
					message.translate("music/lyrics:AND_MORE") +
					" [" +
					message.translate("music/lyrics:CLICK_HERE") +
					"]" +
