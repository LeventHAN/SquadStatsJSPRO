...
class Np extends Command {
	...
	async run(message, args, data) {
		...
		const embed = new Discord.MessageEmbed()
			...
			.addField(
				message.translate("music/np:T_DURATION"),
				message.convertTime(Date.now() + track.durationMS, "to", true),
				true
			)
			.addField(
				message.translate("music/np:T_DESCRIPTION"),
				track.description
					? track.description.substring(0, 150) +
							"\n" +
							message.translate("common:AND_MORE").toLowerCase()
					: message.translate("music/np:NO_DESCRIPTION"),
				true
			)
			.addField(
				"\u200B",
